...
static const char rcsid[] = "$Id: ban.c,v 1.9 2004/09/09 03:42:34 xurinos Exp $";
...
bool check_ban(char *site, int type)
{
   ...
   for ( pban = ban_list; pban != NULL; pban = pban->next )
   {
      ...
      {
         sprintf(log_buf, "Ban: %s  Address: %s", pban->name, host);
         if (type == BAN_ALL || type == BAN_NEWBIES)
         log_string(log_buf);
         return TRUE;
      }
...
void ban_site(CHAR_DATA *ch, char *argument, bool fPerm)
{
   ...
   BUFFER *buffer;
   BAN_DATA *pban, *prev;
   bool prefix = FALSE, suffix = FALSE, allow_ip = FALSE;
   int type;
   ...
   if ( arg1[0] == '\0' )
   {
      ...
      for (pban = ban_list;pban != NULL;pban = pban->next)
      {
         ...
         IS_SET(pban->ban_flags, BAN_IPCHECKING) ? "ipcheck" : "",
   ...
   type = BAN_PERMIT;
   else if (!str_prefix(arg2, "ipcheck"))
   type = BAN_IPCHECKING;
   else
   {
   ...
   if (strlen(name) == 0)
   {
      if (type != BAN_IPCHECKING)
      send_to_char("You have to ban SOMETHING.\n\r", ch);
      else
      send_to_char("You have to stop IP checking on SOMETHING.\n\r", ch);
      return;
   }
   ...
   for ( pban = ban_list; pban != NULL; prev = pban, pban = pban->next )
   {
      ...
      {
         if (type == BAN_IPCHECKING && IS_SET(pban->ban_flags, BAN_IPCHECKING))
         allow_ip = TRUE;
         else
         if (type != BAN_IPCHECKING && IS_SET(pban->ban_flags, BAN_IPCHECKING))
         continue;
         ...
         if (type == BAN_IPCHECKING && !IS_SET(pban->ban_flags, BAN_IPCHECKING))
         continue;
         if ((pban->level > get_trust(ch)) && !allow_ip)
         {
   ...
   if (allow_ip)
   {
      send_to_char("That IP will now be resolved.\n\r", ch);
      return;
   }
   ...
   save_bans();
   if (type == BAN_IPCHECKING)
   sprintf(buf, "%s will no longer be resolved.\n\r", pban->name);
   else
...
void do_allow( CHAR_DATA *ch, char *argument )
{
   ...
   {
      if ( !str_cmp( arg, curr->name ) && !IS_SET(curr->ban_flags, BAN_IPCHECKING))
      {
